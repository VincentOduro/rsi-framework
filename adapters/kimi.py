"""
Kimi (Moonshot AI) adapter — enforcement for opencode + Kimi models.
...
1. **Shell wrapper** (kimi_wrapper.sh): Intercepts file operations at the OS
...
2. **Python tool wrappers** (tool_wrappers.py): For custom agents built on
   Kimi's API directly, use RSISession to wrap tool implementations.
...
class KimiAdapter(BaseAdapter):
    ...
    def platform_name(self) -> str:
        return "Kimi (Moonshot AI) / moonshot-v1-128k"
    ...
    def generate_files(self) -> dict[str, str]:
        return {
            "kimi_wrapper.sh": self._generate_shell_wrapper(),
    ...
    def _generate_shell_wrapper(self) -> str:
        return r"""#!/bin/bash
# RSI Framework — Shell wrapper for Kimi (Moonshot AI)
#
...
echo '[RSI] Kimi shell wrapper active.'
"""

    def _generate_tool_module(self) -> str:
        return '''"""
RSI tool wrappers for Kimi (Moonshot AI) function-calling agents.
...
'''
    ...
    def _generate_instructions(self) -> str:
        rules = RSIRules()
        lines = [
            "# RSI Framework — Standard Work for Kimi (Moonshot AI)",
            "",
            rules.IDENTITY,
            "",
            "## Enforcement",
            "",
            "This project uses the RSI framework. The shell wrapper (`kimi_wrapper.sh`)",

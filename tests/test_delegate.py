...
def test_validate_valid_task(tmp_path):
    ...
    real_issues = [i for i in issues if "not found" not in i.lower() and "already exists" not in i.lower()]
...
def test_write_review(tmp_path):
    ...
    task = {"id": "TASK-005", "description": "Test review", "instruction": "test",
            "acceptance_criteria": ["done"]}
    result = {"changes": {"test.py": "x = 1"}, "proof_wrong": "might fail", "notes": "ok",
              "tokens_used": 100, "latency_seconds": 2.5}
